...
document.addEventListener("click", function (f) {
  if (f.target.tagName == "path") {
      var infocontent = f.target.dataset.name;
      sideInfo.innerHTML = infocontent;
      var datainfocontent = f.target.dataset.info;
      dataInfo.innerHTML = datainfocontent;
        ...
        while(row < 51){
          ...
          var stateInd = 0;
          for(let i = 0; i < 51; i++){
              if(stateData2D[i][0] == f.target.tagName){
                  stateInd = i;
              }
          }
        ...
        var output = ",\n\thome to " + stateData2D[stateInd][2] + " people!\nFemale Population: " + stateData2D[stateInd][4] + " (" + stateData2D[stateInd][5] + "% of total state population)\nLGBTQ Population: " + stateData2D[stateInd][1] + " (" + stateData2D[stateInd][3] + " of total state population)\nRacial Minority Population: " + stateData2D[stateInd][7] + " of total state population\nIn 2020, there were" + "\n\t" + stateData2D[stateInd][8] + " incidents of gender-based hate crimes\n\t" + stateData2D[stateInd][9] + " incidents of race-based hate crimes, and\n\t" + stateData2D[stateInd][10] + " incidents of sexual-orientation-based hate crimes";
